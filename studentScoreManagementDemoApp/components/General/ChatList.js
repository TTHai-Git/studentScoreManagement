  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const filteredUserList = userList.filter(user => user.uid !== currentUser.uid && user.role !== 'admin');

        const fetchLastMessages = filteredUserList.map(async user => {
          const roomId = [currentUser.uid, user.uid].sort().join('_');
          const messagesCollection = collection(database, 'rooms', roomId, 'messages');
          const lastMessageQuery = query(messagesCollection, orderBy('createdAt', 'desc'), limit(1));
  ...
  return (
    <View style={MyStyle.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={{ flex: 1, flexDirection: "row", margin: 10 }}  onPress={() => createChatRoom(item)}>
              ...
              <View style={{ flex: 1, padding: 5 }}>
                <View style={{ justifyContent: "space-between", flexDirection: "row" }}>
                  ...
                  <Text>{ item.lastMessageTime ? item.lastMessageTime.toLocaleTimeString() : 'No messages' }</Text>
                </View>
